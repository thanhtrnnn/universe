...
Viết **toàn bộ báo cáo** 5 chương theo cấu trúc TTCS, ánh xạ sang UP workflows:
- **Claude viết:** Chương 1 + Chương 2 đầy đủ (Requirements + Analysis toàn hệ thống + từng module)
- **3 thành viên viết tiếp:** Chương 3 + Chương 4 (Design + Test) theo module được phân công
...
| Mã UC | UC01–UC14 (14 use cases, 3 module) |
...
| 3 | **Điểm danh & Học vụ** | UC09 Tạo QR, UC10 Điểm danh QR+GPS, UC11 Quản lý điểm danh, UC12 Nhập điểm, UC13 Xem điểm, UC14 Nhắn tin | Trần Xuân Thành |
...
**Module 3: Điểm danh & Học vụ (UC09–UC14)**

| Mục | Tên | Nội dung sẽ có |
|-----|-----|----------------|
| I.1 | UC Module 3 | UC10 Điểm danh <<include>> xác minh QR + xác minh GPS |
| II.1 | Kịch bản UC10 Điểm danh | 12 bước; ngoại lệ: QR hết hạn, ngoài phạm vi, đã điểm danh |
| II.1 | Kịch bản UC12 Nhập điểm | 8 bước; ngoại lệ: điểm ngoài khoảng hợp lệ |
| II.2 | Mô hình hóa lớp | Attendance n-1 Schedule; Grade n-1 Enrollment; Message (MongoDB) |
| II.3 | Sơ đồ lớp phân tích | Boundary: AttendancePage, GradePage; Entity: Attendance, Grade, Message |
...
| Mục | Tên | Nội dung |
|-----|-----|----------|
| III.1 | Thiết kế lớp Entity | `Attendance` (qrToken, gpsLat, gpsLon, distance), `Grade` (type, weight) |
| III.2 | Thiết kế CSDL | Bảng `attendances`, `grades` + Collection MongoDB `messages` |
| III.3.1 | Giao diện | Wireframe: QRScanPage, AttendanceTrackingPage, GradeEntryPage |
| III.3.2 | Sơ đồ lớp TK | DAO cho Attendance, Grade + MessageDAO (MongoDB) |
| III.3 | Thuật toán | Haversine formula, HMAC QR token, grade calculation |
| III.4 | Tuần tự TK | 2 biểu đồ: UC10 + UC12 |
| IV | Test | 10 test case UC10 + UC12 |
...
4. **Mã UC xuyên suốt**: UC01–UC14 không thay đổi giữa các chương
5. **PlantUML**: `left to right direction` + `skinparam linetype ortho`
6. **File output**: `docs/module1_auth_users.md`, `docs/module2_academic.md`, `docs/module3_attendance_grades.md`
